    ...
    try:
        with open(filepath, 'r', encoding='utf-8') as f:
            ...
            if content:
                ...
                # Make sure older versions of user_data.json still work
...
def practice_answer(question_num):
    ...
    # Send to learning page if wrong
    review_lessons = {
        1: 3,   # Pong / melds
        2: 4,   # Winning hand
        3: 3    # Kong / melds
    ...
    review_lesson = review_lessons.get(question_num)
...
def resume():
    ...
    # If no quiz answers yet, begin at quiz 1
    if not answers:
...
def results():
    ...
    # Save final score and completion time
    user_data['score'] = score
    if user_data.get('finished_at') is None:
        user_data['finished_at'] = datetime.now().isoformat()
    save_user_data(user_data)

    # Compute minutes spent
    minutes_spent = None
    started = user_data.get('started_at')
    finished = user_data.get('finished_at')

    if started and finished:
        try:
            start_time = datetime.fromisoformat(started)
            finish_time = datetime.fromisoformat(finished)
            minutes_spent = round((finish_time - start_time).total_seconds() / 60, 1)
        except ValueError:
            minutes_spent = None
